...
- [x] 小口ページを開いてもカメラが起動しない → **localhost:8503 の実機で確認済**（camera_input は無く「カメラを起動」ボタンのみ。押すとカメラ＋「カメラを閉じる」が出る）
- [x] 原価・売上まとめに「原価」「売上」のタブが出て、区分が混ざらない → **実機で確認済**（原価39件 ¥4,119,683 ＝ KPIの原価と一致／売上1件 ¥290,703 ＝ KPIの売上と一致）

> 検証は 8503 で行い、オーナーが使う 8502 には触れていない。検証後 8503 のみ停止済み。
