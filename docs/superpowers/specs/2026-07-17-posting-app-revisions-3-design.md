...
| 7a | 支払形態に応じた報酬の**自動計算** | **やる**（当初スコープ外だったが 2026-07-17 に追加）。下記「報酬の自動計算」節 |
| 7b | 計算式 | 日当=日当額×日数／歩合=単価×部数（現行のまま）／時給=時給×時間／月給=月額×1。**すべて「単価×数量」の形に収まる**ので既存の明細の仕組みをそのまま使う |
| 7c | 単価の置き場所 | **時給額・月額はマスタ**（人ごとに決まっているため）。**歩合単価は明細で手入力**（号ごとに違うため）。日当は業務名ごとにマスタ |
| 7d | 自動で入った単価の上書き | **明細で手変えできる**（マスタ側は変わらない） |
...
| `distributors` | `hourly_rate` | INTEGER | 時給額（支払形態=時給のとき） |
| `distributors` | `monthly_rate` | INTEGER | 月額（支払形態=月給のとき） |
| `contract_invoices` | `pay_type` | TEXT | **登録時点の支払形態のスナップショット**（下記） |
| `contract_invoice_lines` | `copies` | INTEGER | 配布部数。**歩合以外**（日当・時給・月給）のときだけ使う |

`distributors.kind`（区分: 業務委託／自社社員／アルバイト）は**変更せず残す**。

#### なぜ `contract_invoices.pay_type` にスナップショットを取るのか

支払形態はマスタの値なので、後から変えられる（例: 山田さんが日当→歩合に変わる）。
マスタの現在値を見て過去の請求を描画すると、**登録時は「3 日」だった数量が、後日「3 枚」に化ける**。
これは「停止中方式」で過去データの名前を守るのと同じ理屈で、**請求の単位は登録時点の事実として固定**する必要がある。
そのため登録時に `pay_type` を請求ヘッダへ書き込み、表示・報告書はそれを使う。
既存データ（第2弾以前の請求）は `pay_type` が NULL になるが、NULL は歩合と同じ扱い＝**現行の「枚」表示のまま変わらない**（後方互換）。
...
支払形態が変わっても計算は「**単価 × 数量**」のままで、**変わるのは数量の単位と、単価がどこから来るか**だけ。
そのため `contract_invoice_lines` の **数量・単価・合計の関係（合計=数量×単価）は変えない**。
足すのは配布部数を持つ `copies` 列だけで、これは計算には使わず報告数の表示にだけ使う。

| 支払形態 | 単価の出どころ | 数量の意味 | 単位 |
|---|---|---|---|
| 日当 | マスタの業務名ごとの日当額（自動・手変え可） | 日数 | 日 |
| 歩合 | 明細で手入力（号ごとに違うため） | 部数 | 枚（現行のまま） |
| 時給 | マスタの時給額（自動・手変え可） | 時間 | 時間 |
| 月給 | マスタの月額（自動・手変え可） | 1 固定 | 一式 |

既存の `unit_for` / `qty_label` を **`pay_type` を受け取る形に拡張**する（既定 None＝歩合＝現行動作）。

```python
def unit_for(remark, pay_type=None) -> str:
    """数量の単位。交通費・手当・その他は今まで通り数を数えないので『一式』。
    配布・挟み込みのときだけ支払形態で単位が変わる。"""
```

- `remark` が 配布／挟み込み **以外**（交通費・手当・その他）→ 支払形態によらず **一式**（現行維持）。
  日当の人でも交通費の行は「一式」であるべきなので、支払形態で一律に上書きしてはいけない。
- `remark` が 配布／挟み込み → 日当=日／時給=時間／月給=一式／歩合・NULL=枚

#### 配布部数（`delivered_copies`）の扱い

`delivered_copies` は 配布／挟み込み 行の数量を合計して「配布部数」として出しており、
**業務委託登録の画面のメトリクスと、報告書兼請求書Excelの報告数**に使われている。

歩合以外では数量が部数ではない（日当なら「3」は3日）ため、そのまま合計すると「配布部数 3 部」と誤表示になる。
→ **明細に `copies`（部数）列を持たせ、歩合以外はそこから部数を取る**（2026-07-17 オーナー決定）。

```python
def line_copies(line, pay_type=None):
    """その明細行の配布部数。歩合(と未設定)は数量がそのまま部数。
    日当・時給・月給は数量が日数/時間なので、別列の copies を使う。"""
```

- **歩合・NULL** → 数量（`report_qty`）がそのまま部数＝**現行動作のまま**
- **日当・時給・月給** → `copies` 列の値（未入力なら 0）
- 配布／挟み込み **以外**の行（交通費・手当・その他）は部数を数えない＝現行どおり

UI では**歩合のときに「部数」列を出さない**（数量がそのまま部数なので、出すと同じ数を2回打たせることになる）。

### store に追加する関数（自動計算ぶん）

- `add_contract_invoice(..., pay_type=None)` — 登録時の支払形態を保存
- `list_daily_rates(distributor_id)` — 日当の業務名候補と金額を明細に供給
...
### ② 業務委託登録（`pages/02_業務委託登録.py`）＝ 自動計算の入口

- 配布員を選んだら、その人の**支払形態を画面に表示**する（「支払形態: 日当」）。何の単位で入力するのかを明示するため
- 明細の列を支払形態で切り替える：

| 支払形態 | 明細の列 |
|---|---|
| 歩合・未設定 | 案件／種別／数量(枚)／単価／その他の案件名　＝**現行のまま** |
| 日当 | 案件／種別／**業務**／単価(自動)／数量(日)／**部数**／その他の案件名 |
| 時給 | 案件／種別／単価=時給(自動)／数量(時間)／**部数**／その他の案件名 |
| 月給 | 案件／種別／単価=月額(自動)／数量(1固定)／**部数**／その他の案件名 |

- 「業務」列は**マスタに登録した業務名からの選択**。選ぶと単価にその日当額が自動で入る（手変え可）
  ※ 業務名は**単価を引くためだけに使い、保存しない**。報告書に載る区分は今まで通り「種別」（配布／挟み込み／交通費／手当／その他）。
    種別は `is_delivery` の判定に使われる既存の軸なので、業務名で置き換えたり兼用したりしない
- 時給・月給は配布員を選んだ時点で単価にマスタの金額が自動で入る（手変え可）
- 合計・ご請求金額の計算式は**変わらない**（単価×数量）
- 登録時に `pay_type` を請求ヘッダへ、`copies` を明細へ保存する

### ② マスタ管理（`pages/05_マスタ管理.py`）

- タブ名「配布委託先」→**「業務委託」**。画面内の文言（`配布委託先はまだ登録されていません` など）もあわせて変更
- 買掛先タブ：追加フォームに「既定の原本区分」selectbox を追加。一覧にも列を追加
- 業務委託タブ：追加フォームに「振込先」（`text_area`）・「支払形態」（selectbox）・**「時給額」「月額」**を追加。一覧にも列を追加
  ※ 時給額・月額は該当する支払形態のときだけ使う。フォーム内で出し分けはできない（Streamlitの仕様）ため両方出し、
    caption で「支払形態が時給／月給のときだけ使います」と補足する
...
- `tests/test_posting_logic.py`：`resolve_original_status`、`master_delete_action`、
  `unit_for` / `qty_label`（4つの支払形態 × 種別の組み合わせ・**pay_type 未指定で現行動作が変わらないこと**）、
  `line_copies`（歩合は数量／それ以外は copies 列）、`delivered_copies`（支払形態別）
- **後方互換のテストを必ず置く**：`pay_type` が NULL の既存請求が、今までと同じ「枚」表示・同じ部数で出ること
...
- 実績（勤怠・出面）からの日数・時間の自動取り込み。数量は今まで通り人が入力する
