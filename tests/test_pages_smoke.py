...
def test_issue_page_has_no_meeting_uriagehyo_export(db, monkeypatch):
    """🔴 2026-08-28 依頼5: 会議用売上表の出力は「大阪支社売上」ページに一本化した。
    号別明細から旧機能(8dda423)を削除したので、二重に残っていないこと。
    2か所から別レイアウトのExcelが出ると、どちらが正か分からなくなる。"""
    import io
    ...
    assert not at.exception
    ids = [b.id for b in at.get("download_button")]
    assert not any("dl_uriage" in i for i in ids), "号別明細に会議用売上表の出力が残っている"
    assert any("dl_genka" in i for i in ids), "号原価まとめの出力まで消してはいけない"
    for b in seen:
        assert "売上表用出力" not in pd.ExcelFile(io.BytesIO(b)).sheet_names
